    ...
    for (int i = 0; i < n; i++)
    {
        if (factors.find(arr[i]) == factors.end())
        {
            for (int j = 1; j * j <= arr[i]; j++)
            {
                if (arr[i] % j == 0)
                {
                    factors[arr[i]].push_back(j);
                    if (arr[i] / j != j)
                    {
                        factors[arr[i]].push_back(arr[i] / j);
                    }
                }
            }

            sort(factors[arr[i]].begin(), factors[arr[i]].end());
        }
    }
